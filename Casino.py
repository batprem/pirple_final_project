...
class Casino:
		...
		def __init__(self):
			...
			for pips in ['♣', '♦', '♥', '♠']:
				for index in range(1, 14):
					if index == 1:
						card = Casino.Card(pips, 'A')
					elif index == 11:
						card = Casino.Card(pips, 'J')
					elif index == 12:
						card = Casino.Card(pips, 'Q')
					elif index == 13:
						card = Casino.Card(pips, 'K')
					else:
						card = Casino.Card(pips, str(index))
	...
	class Table:
		...
		def __init__(self, player_1, player_2):
			...
			self.deck = Casino.Deck()
			print(f"Match making between {self.player_1.name} and {self.player_2.name}")
			self.match_player()
			self.score_master = Casino.ScoreMaster()
